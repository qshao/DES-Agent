#!/usr/bin/env bash
# Ni2+/Co2+ selectivity-DES pipeline with Nemotron-3-Nano (Ollama)
#
# Goal: find five HBD/HBA ligands selective for Ni2+ over Co2+, then find
# deep eutectic solvents containing the top three ligands with Tm < 350 K,
# low viscosity (<=200 cP), and hydrophobic character where possible.
#
# Prerequisites:
...
OUTPUT_DIR="${1:-runs/ni_co_selectivity_des_nemotron_001}"
mkdir -p "${OUTPUT_DIR}"
...
python -m des_multi_agent.cli \
  ...
  --llm-config llm.ni_co_nemotron.yaml \
  \
  `# --- Save outputs ---` \
