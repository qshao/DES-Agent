...
22. Concurrent LLM review calls
    - The item 21 benchmark surfaced the real bottleneck: DES-Agent's own orchestration never sent concurrent requests, so vLLM's continuous batching had nothing to batch — the server logs showed `Running: 1 reqs` throughout. A new `run_concurrent(items, call, max_workers=8)` helper (`des_multi_agent/concurrency.py`, `ThreadPoolExecutor`-based, never raises — failures come back as `CallResult.error`) now parallelizes all 4 sequential per-candidate/per-ligand LLM review loops: `orchestrator.py`'s `_review_top_candidates` and chemistry-advisor loop, and both `metal_binding_screen.py`/`metal_binding_selectivity.py`'s `review_ligand` loops. Ollama and the hosted API backends (`openai`/`gemini`/`custom_http`) are unaffected — they simply receive up to 8 concurrent requests instead of 1 at a time.
    - `selectivity_des_pipeline.py`'s per-ligand loop was deliberately left untouched — each iteration runs a whole nested multi-cycle search, not a single LLM call, so parallelizing it is a separate, larger change.
    - **Code-review fix**: the chemistry-advisor loop's refactor left `advisor_assessments.extend(res.value)` outside any try/except, whereas the original code had the whole `.extend(provider_call())` expression inside one try/except. A provider returning a non-iterable from `assess_candidate_chemistry` would have crashed `run_search_report` instead of degrading to a warning like every other LLM failure mode in this codebase. Fixed by wrapping the `.extend()` call in its own try/except, restoring the original graceful-degradation contract.
    - **Test-coverage fix**: the concurrency-proof tests for 3 of the 4 call sites originally validated `run_concurrent`'s behavior in isolation (hand-copying the loop body) rather than driving the real production function, so a future revert to sequential execution would have gone undetected by any test. Rewritten to call the actual `run_search_report`/`run_metal_binding_screen`/`run_metal_selectivity_screen` functions with a concurrency-tracking or barrier-based fake provider.

23. Full example re-run (2026-07-06)
    - Every example in `examples/` was re-run end to end against a live Ollama instance and the current codebase (grounding layer, analogue expansion, concurrency support all included). See [`docs/example-run-report-2026-07-06.md`](example-run-report-2026-07-06.md) for full timings and findings; `examples/README.md`'s stale "predate this feature" note was updated and the frozen regression baselines under `tests/fixtures/example_benchmark_baselines/` were resynced.
    - Surfaced two findings, detailed in the report: (1) `task_execute`'s qwen3.6 router call occasionally returns a JSON job missing required fields — genuine sampling non-determinism at `temperature=0.2` rather than a code bug (`task_router`'s identical routing/validation code path succeeded in the same run), a pre-existing prompt-design gap left as a follow-up rather than fixed in-place; (2) offline "deterministic" examples are still byte-reproducible run-to-run, but several outputs had drifted in content since their baselines were last frozen (grounding verdicts, analogue-expanded candidates, new `trajectory.json` artifact) — all resynced in this pass.
    - Separately, a stray `tests` package installed by `MDAnalysisTests` in site-packages was shadowing the repo's local `tests/` package (regular packages always win over namespace packages regardless of `sys.path` order), breaking `tests/test_benchmarks_examples.py`'s `tests.fixtures` import and blocking the whole benchmark suite. Fixed by adding `tests/__init__.py` and `tests/fixtures/__init__.py`.
...
5. Router prompt robustness (example re-run finding, 2026-07-06)
   - `ROUTER_SYSTEM_PROMPT` tells the model to "use existing CLI field names" without ever listing them, so a model that doesn't already know the `RouterJob` schema can occasionally invent its own field names (`target_molecule` instead of `component_a`), which get silently dropped by the strict field filter and fail required-field validation. Observed as an occasional, non-deterministic failure on `task_execute` (not every call) rather than a hard-broken path. Fix: enumerate the actual field names in the prompt, and/or give `task-router`/`task-execute` the same client-side default-substitution fallback the plain-language example scripts (`_normalize_router_job`) already have.

6. Backend-aware concurrency tuning (example re-run finding, 2026-07-06)
   - `run_concurrent`'s fixed `max_workers=8` (item 22) assumes a backend that can truly serve that many requests in parallel (vLLM). Against Ollama, which serves a single loaded model with much more limited real parallelism, 8 "concurrent" requests mostly queue — helping little and, at high candidate counts, risking timeouts unless `timeout_seconds` is sized generously. `timeout_seconds` in the example configs is already raised to `300.0`; no timeouts were observed in the 2026-07-06 re-run at that setting, but a backend-aware `max_workers`/timeout policy would be more robust than a fixed constant. Consider detecting the backend and adjusting `max_workers`, or scaling `timeout_seconds` with candidate count specifically for Ollama configs.
