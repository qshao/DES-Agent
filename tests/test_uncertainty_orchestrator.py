...
def test_run_search_report_reports_uncertainty_failure(monkeypatch, tmp_path):
    monkeypatch.setattr(
        orchestrator,
        "generate_candidates",
        lambda component_a, n, constraints=None: [
...
""",
    ...
    assert outcome.annotated_results[0].trust_score == 0.0
    assert any("Uncertainty estimation failed" in warning for warning in outcome.llm_warnings)
